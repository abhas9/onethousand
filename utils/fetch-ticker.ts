...
export const fetchTicker = async (ticker: string): Promise<TickerResults> => {
  const { data } = await axios.get<YahooHistoryResult>(
    `https://query2.finance.yahoo.com/v8/finance/chart/${ticker}?formatted=true&crumb=ZBpLWWzBYL9&lang=en-IN&region=IN&includeAdjustedClose=true&interval=1d&period1=-952374400&period2=2610841600&events=div%7Csplit&useYfid=true`
  );
...
export const fetchTickerProfile = async (
  ...
  const { data } = await axios.get<YahooProfileResult>(
    `https://query1.finance.yahoo.com/v7/finance/quote?formatted=true&crumb=ZBpLWWzBYL9&lang=en-IN&region=IN&symbols=${ticker}&fields=shortName%2CmarketCap%2CunderlyingExchangeSymbol`
  );
